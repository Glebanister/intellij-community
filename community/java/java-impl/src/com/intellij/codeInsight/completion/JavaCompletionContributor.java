    ...
    if (position instanceof PsiIdentifier) {
      ...
      if (smart) {
        ...
        if (!JavaKeywordCompletion.AFTER_DOT.accepts(position)) {
          CompletionKindWithMutableFiller ckExpectedInfos = withEmptyFillFunction("expected_info");
          ckExpectedInfos.setVariantFiller(() -> {
            for (ExpectedTypeInfo info : expectedInfos.get()) {
              ClassLiteralGetter.addCompletions(new JavaSmartCompletionParameters(parameters, info), result, matcher);
            }
          });
          ckExecutor.addKind(ckExpectedInfos, session);
        }
      }
